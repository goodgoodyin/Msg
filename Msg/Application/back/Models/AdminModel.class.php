...
class AdminModel extends BaseModel{
	...
	public function ReplyActInfo($message,$parent_id,$main_id,$level,$username) {
		$time=date("Y-m-d H:i:s");
		$sql="insert into _act_reply (parent_id,main_id,nickname,connect,time,level) values ($parent_id,$main_id,'$username','$message','$time',$level)";
		return $this->_dao->exec($sql);
	}//添加回复
	...
	public function ShowReplyAdminInfo3($id) {
		$sql="select * from _act_reply where main_id=$id";
		return $this->_dao->GetRows1($sql);
	}
	...
	public function ReplyAdminInfo($message,$parent_id,$main_id,$level,$username) {
		$time=date("Y-m-d H:i:s");
		$sql="insert into message_reply (parent_id,main_id,nickname,connect,time,level) values ($parent_id,$main_id,'$username','$message','$time',$level)";
		return $this->_dao->exec($sql);
	}//添加回复
