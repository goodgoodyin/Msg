...
class MySQLDB{
	...
	private function __construct($config){
		//先将这些基本的连接信息，保存起来！
		$this->host = !empty($config['host']) ? $config['host'] : "127.0.0.1";	//考虑空值情况，使用默认值代替
		...
		$this->dbname = !empty($config['dbname']) ? $config['dbname'] : "_message" ;
	...
	function exec($sql){
		...
		return $result;	//因为是增删改语句，直接返回true就可以
	}
